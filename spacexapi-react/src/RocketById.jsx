...
function RocketById(props) {
  const [ loading, setLoading ] = useState(true);
  const [ rocket, setRocket ] = useState(undefined);
  let params = useParams();
  let rocketId = params.id;
  useEffect(() => {
    axios.get(`https://api.spacexdata.com/v4/rockets/${rocketId}`).then(({ data }) => {
      setRocket(data)
      setLoading(false)
    }).catch((e) => {
      setLoading(false)
  ...
  if (loading) return <div>Loading Rocket...</div>
  else{
    return (
      <>
        ...
        { rocket.flickr_images.length != 0 && <div><img className= "shrinkMain" src={rocket.flickr_images[0]} alt={`${rocket.name}`}/></div>}
        <div>
          <h2>Details</h2>
