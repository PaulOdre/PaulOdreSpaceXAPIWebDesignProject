...
function PayloadById(props) {
  const [ loading, setLoading ] = useState(true);
  const [ payload, setPayload ] = useState(undefined);
  let params = useParams();
  let payloadId = params.id;
  useEffect(() => {
    axios.get(`https://api.spacexdata.com/v4/payloads/${payloadId}`).then(({ data }) => {
      setPayload(data)
      setLoading(false)
    }).catch((e) => {
      setLoading(false)
  ...
  //You know how tempted I was to make the text "PayLoading..."?
  if (loading) return <div>Loading Payload...</div>
  else{
    return (
      <>
        ...
        <div>
          ...
          <p>Launch: <NavLink to={`/launches/${payload.launch}`}>{payload.launch ? payload.launch : "N/A"}</NavLink></p>
